...
def infer_metadata(file_path):
    ...
    meta = {"source": "Unknown", "type": "Unknown"}
    if "pubmed" in name:
        meta["source"], meta["type"] = "PubMed", "scientifique"
    elif "trials" in name:
        meta["source"], meta["type"] = "Trials", "essai clinique"
    elif "cancergov" in name:
        meta["source"], meta["type"] = "CancerGov", "recommandation"
    elif "who" in name:
        meta["source"], meta["type"] = "WHO", "statistiques"
    elif "patient" in name:
        meta["source"], meta["type"] = "CancerNet", "vulgarisation"
...
def main():
    ...
    files = [f for f in files if f.endswith((".json", ".txt"))]
    ...
    output_file = os.path.join(BASE_DIR, "processed_embeddings.json")
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
